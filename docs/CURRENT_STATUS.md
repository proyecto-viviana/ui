...
| Visual state coverage         | `180` official states tracked, `49` with current React/Solid visual evidence, `32` with strict pair-diff tests  | The visual harness exists, but most states are not strict parity yet.                                                          |
...
3. Continue with the next styled S2 gap from the report. The support-component
   sweep exposed by Form remains comparison-live, while Avatar, AvatarGroup,
   Badge, Divider, Image, Link, Meter, Skeleton, and StatusLight are playbook
