...
| Spectrum S2 catalogue         | `vp run comparison:report:gaps`: `69` official entries tracked, `23` live on both sides, `46` missing/gap     | Styled parity is the main unfinished surface.                                                                                  |
| Visual state coverage         | `152` official states tracked, `38` with current React/Solid visual evidence, `8` with strict pair-diff tests | The visual harness exists, but most states are not strict parity yet.                                                          |
| S2 export surface             | `vp run comparison:report:exports`: `87` of `208` React S2 value exports missing                              | Root catalogue component exports are present; support exports such as contexts, slots, hooks, and helpers are still missing.   |

## Main Gaps

Styled S2 entries still missing or blocked:

Accordion, ActionBar, ActionMenu, Avatar, AvatarGroup, Badge, Breadcrumbs,
...
Dialog, Disclosure, Divider, DropZone, Form, Icons, IllustratedMessage,
Illustrations, Image, InlineAlert, Link, ListView, Menu, Meter, Popover,
ProgressBar, ProgressCircle, RangeCalendar, RangeSlider, Skeleton, StatusLight,
TableView, Tabs, TagGroup, TimeField, Toast, Tooltip, TreeView.
...
2. For date/time work, continue the cluster rather than treating each widget as
...
5. Keep comparison CSS limited to app shell, controls, panels, and screenshot
