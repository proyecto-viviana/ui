...
| Visual state coverage         | `178` official states tracked, `49` with current React/Solid visual evidence, `32` with strict pair-diff tests  | The visual harness exists, but most states are not strict parity yet.                                                          |
...
3. Continue with the next styled S2 gap from the report. The support-component
   sweep exposed by Skeleton, Image, and Form passes is comparison-live through
   Form, while Avatar, AvatarGroup, Badge, Divider, Link, Meter, and StatusLight
