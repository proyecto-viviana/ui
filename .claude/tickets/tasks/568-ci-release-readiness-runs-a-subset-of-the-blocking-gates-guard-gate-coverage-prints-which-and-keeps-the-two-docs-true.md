...
history:
  ...
  - {
      state: merged,
      at: 2026-09-22,
      note: "2026-09-22. A newline separates commands like &&, || and ; do, and invokedScripts returns each script once, in first-seen order. A body with the script below line 1 was accepted at 8f6cf382, rejected at 972aff51, and accepted again in this commit. stepRunsLocally: deleted. 15:21 a probe of 1003 typecheck-step bodies found 0 where problems was empty and the two extra conditions changed the count, so reached.has(leg) is the whole rule. The sentence: ci:release-readiness runs 11 of 44 blocking gate steps locally across the six Certification Gates jobs; the other 33 run only there; 45 steps are runner plumbing (scripts/gate-coverage.json). 15:32 vp run guard:gate-coverage -> 0 and printed that sentence. 15:32 vp test run scripts/check-gate-coverage.test.ts --maxWorkers=2 -> 0, 17 passed. 15:32 vp run test:ci-guard-contracts -> 0. 15:32 vp run check -> 0. 15:33 vp fmt --check on the mjs, the test, and both tickets -> 0. 15:33 vp lint on those paths -> 0. 15:33 vp run docs:generate -> 0 (roadmap.md and status.md unchanged). 15:33 vp run docs:check -> 0. 15:35 vp fmt --check on the mjs, the test, and both tickets -> 0. 15:35 vp lint on those paths -> 0. 15:35 vp run docs:generate -> 0 (roadmap.md and status.md unchanged). 15:35 vp run docs:check -> 0. 15:36 vp fmt --check on the mjs, the test, and both tickets -> 0. 15:36 vp run docs:check -> 0 after this note.",
    }
---
