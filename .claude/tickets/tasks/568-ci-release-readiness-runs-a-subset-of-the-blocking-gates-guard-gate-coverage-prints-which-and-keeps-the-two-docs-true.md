...
`jobBlock` is still a second copy: `scripts/check-gate-coverage.mjs:48` and `scripts/test-ci-guard-contracts.mjs:65`. `scripts/check-gate-server-reuse.mjs` and `scripts/check-workflow-pins.mjs` do not read workflow jobs, so there is nothing to extract in one move.
