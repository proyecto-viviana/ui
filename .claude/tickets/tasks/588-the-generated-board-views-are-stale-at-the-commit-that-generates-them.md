...
history:
  ...
  - {
      state: verified,
      at: 2026-09-21,
      note: "review of that flip, and the finding is real: `verified` was written with one of three Done-when clauses unproved. The run and the local EXIT=0 were proved; the frontier clause was not, and as written it could not be. `grep -nE 'ladder|frontier|35(6|5)[0-9]{6}' .claude/current/status.md` EXIT=1, no match, at `b6ea736a`; the file is generated, carries `Do not edit this file`, and `.claude/current/README.md` puts work state only in `.claude/tickets`. So two corrections, the second of which is this seat editing acceptance text after the fact and says so rather than doing it quietly. (1) The frontier is one line on #544 now, at the end of the `Soft-launch cut` section's first bullet, naming the run and the step it stopped at - the fact was already in that bullet inside `b6ea736a`, which is why the flip is under-proved rather than false, but nothing said it was the frontier and no clause pointed at it. (2) The third Done-when clause names that place instead of `status.md`. The reviewer's other option, rolling `verified` back to `merged`, is not available: the lifecycle runs forward only, OWNERS deviation 5, which is why #546, #576 and #582 carry notes instead of moves. Re-measured in this seat, not quoted: `gh run view 35646778662 --json jobs`, job 106489054009 `certification-gates`, 47 steps, step 37 `docs:check` `success`, step 38 `comparison parity (strict)` `failure`, steps 39 `axe full audit` and 40 `guard upstream-freshness` `skipped`. Stays `verified`",
    }
---
...
with EXIT=0. The ladder frontier is written where work state is allowed to
live — the campaign path on #544, not the generated `status.md` — and names
the step the run actually stopped at, with its run id.
