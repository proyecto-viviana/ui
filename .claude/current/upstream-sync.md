...
- [ ] **4. no-overlap components** — gridlist, tooltip, colorswatch, selectboxgroup, filetrigger, dropzone (no role overlap at all today). NB: gridlist's `{rowgroup}` gap is the same one as item 3b — fix together.
