...
This is partly an **owner decision**: the project is a parity port until the
release policy says otherwise, so promoting `ui` for client apps is exactly the
call steering left open.
...
- ~~**Build the out-of-workspace fixture**~~ ✔ `scripts/consume-pack-smoke.mjs`
  (reused by UC-01/03/04/05): scaffolds a throwaway app _outside_ the workspace,
...
- **Built-package smoke extended** (`scripts/consume-pack-smoke.mjs`): against the
  ...
  specifier (44/44). It deliberately checks _resolution + file presence_, not
...
_Shares the `./style_` export surface with UC-01 — same work-stream.\*
...
> Part B is blocked on a design call (see _Part B status_ below) — it is the one
...
Viviana palette (`viviana-tokens.css`) uses a _different_ vocabulary
...
- **Smoke extended** (UC-03 block in `consume-pack-smoke.mjs`): against the real
  ...
  `style.css` sidecar is absent, and the shipped `styles.css` is the _complete_
...
_Same "deep-subpath" fix as UC-01, one layer down — design together._
...
- **Guard (better than the planned one):** `guard:jsx-deopt-size`
  ...
  500 KB Babel deopt threshold — the _actual_ failure mode, more robust than the
...
_Surfaced by UC-05's `guard:jsx-deopt-size`; same class of fix, one layer up._
...
it to an entry _roots_ that namespace and defeats tree-shaking (a 631 KB chunk over
