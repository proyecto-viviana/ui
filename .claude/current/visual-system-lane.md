...
Status: **closed at merge.** The lane's work — continued on
`design/visual-system-claude-v2` after the v1 branch named below — landed on
`main` on 2026-07-22 as a squash-merge (owner-directed; the owner authorized the
session to perform the squash on their behalf). The paired Education-repository
half is frozen as of the same date; the external design repo is a read-only
reference. This document remains as the provenance record. Ongoing Glasselated
work proceeds on `main` per `glasselated-port.md`.
Update when: the provenance record needs correction.
