...
> **Press-path Phase 2 completed (2026-06-23).** The client-readiness `UC-NN`
> track landed (`ui-client-contract.md`), and **T-57** (focusable-utils port)
> and **T-58** (autocomplete `autoFocusOnMount` + Arrow inline-nav, hook level)
...
> wrapper-surface evidence. **T-59** (table focus-ring) is now done.
> Note: T-58's
...
- [x] **T-51** ✔ RAC — **`replace`-mode action model (architectural epic).** Wire upstream's single-click-selects / double-click-acts split so the row action becomes the _secondary_ action under `selectionBehavior: 'replace'` with selection enabled. Route `gridlist/createGridListItem.ts`, `tree/createTreeItem.ts`, and `table/createTableRow.ts` activation through a **press-based path** (not raw pointer events), thread `hasPrimaryAction` / `hasSecondaryAction` + `pointerType`, and add the `onDoubleClick` secondary action and the touch **long-press → `setSelectionBehavior('toggle')`** switch. Evidence: the `selectionBehavior: 'replace'` section in upstream-sync.md. **High risk / cross-hook — scoped 2026-06-20 in `press-path-epic.md`** (Phases 0–2 there: T-52 foundation → build `createSelectableItem` → migrate the three item hooks; T-56 falls out in Phase 2). **Ported 2026-06-23:** `createGridListItem`, `createTreeItem`, and `createTableRow` now route through `createSelectableItem` with discriminating tests for replace-mode single-click-selects, mouse double-click action, Enter action on keyup, Space selection on the focused row, and disabled-for-selection actionability. TableView comparison evidence (`e2e/tableview-visual.spec.ts`) now passes the controlled selection/action/link scenario after the Solid `createPress` composed-path containment fix and S2 default selection-timing alignment.
...
- [x] **T-56** ✔ RAC — **`disabledBehavior: 'selection'` onAction on the _pointer_ path — ported.** A `disabledBehavior: 'selection'` item is selection-blocked but still actionable, so upstream fires `onAction` on **both** click and keyboard (`useSelectableItem.ts`: selection is blocked by the resolved disabled check, but `allowsActions` remains true for the item press path). The Phase 2 press-path migration now routes the affected hooks through `createSelectableItem`: `createGridListItem`, `createTreeItem`, `createTableRow`, `listbox/createOption`, and `menu/createMenuItem`. Selection-disabled rows/options/menu items remain unselectable while pointer and keyboard actions still fire, matching upstream's split. Menu was reconciled separately because upstream composes `useSelectableItem` with `shouldSelectOnPressUp: true`, `allowsDifferentPressOrigin: true`, and `linkBehavior: "none"` but keeps menu `onAction`/close/submenu behavior in the menu layer; our `createMenuItem` mirrors that split. Tests added/updated across `createSelectableItem`, `createMenu`, and component Menu cover disabled-for-selection pointer action, different-origin pointer release, and the Enter-vs-pointer close defaults for multiple-selection menus. The Solid keyboard `.click()` modality guard needed for Menu is documented in `.claude/reference/patterns.md`.
...
- [x] **T-59** ✔ S2 — **TableView cell focus-ring `--topFocusRing` offset (focus-ring re-architecture).** Ported 2026-06-24. Upstream's `CellFocusRing` is an absolutely positioned ring `div` whose `top: var(--topFocusRing, 0)` extends to `-1px` (overlapping the divider above) for all but the first row and whose `bottom` remains `0`; upstream rows separately expose the matching `--topFocusRing`/`--bottomPosition` vars plus the row `focusIndicator` `::after` overlay. Our S2 `TableView` now mirrors that rendering model: body cells, selection cells, headers, and `EditableCell` render a presentational `CellFocusRing` child on focus-visible; rows publish the custom props, distinguish the first row's top offset, and inject the raw row `::after` focus indicator only when needed. The old direct `focusRing()` cell outline is gone because a uniform outline cannot express the asymmetric top extension. Evidence: `packages/solid-spectrum/test/Table.test.tsx` covers row overlay CSS, first-row class branching, header rings, body-cell rings, and selection-cell rings; `packages/solid-spectrum/test/EditableCell.test.tsx` covers editable-cell rings; `apps/comparison/e2e/tableview-visual.spec.ts` focuses the same Budget row/rowheader in React and Solid, asserts the current React fixture's shared `CellFocusRing` structure, guards that the installed React S2 fixture is still pre-row-vars, and asserts Solid's vendored-upstream `--topFocusRing`/`--bottomPosition`, row `::after`, and `CellFocusRing` offset geometry. **Split from T-46** (T-46 landed the divider-transparency delta; this closes the focus-ring half).
...
`selection-onselect-layer-split.md`). **T-51** is now ✔ for the three row hooks
(`createGridListItem`, `createTreeItem`, `createTableRow`) through the shared
selectable-item press path. **T-56** (pointer-path `onAction`) is now ✔ after
...
**T-46** (TableView re-sync) is now closed through its two valid branches: the
divider-transparency delta landed, the row-`borderBottom` move is N/A under our
real-`<table>` `border-collapse: separate` DOM, and the `CellFocusRing`
top-offset branch is closed by **T-59**. **T-47** (ButtonGroup overflow) is done — the
...
done; **T-59** table focus-ring is closed; **T-51**'s
