...
tasks:
  ...
  - id: launch-seo-surface
    ...
    resolution: >-
      All 70 indexable routes now set their own head through one helper,
      `apps/web/src/seo.ts`, which emits title, description, canonical, Open Graph
      and Twitter card tags from a single `SITE_URL` constant — the origin is
      `https://proyectoviviana.org`, owner-supplied, and a domain change is a
      one-line edit. The 47 `<DocPage>` routes lift the title and description they
      already render on the page, so the tab and the H1 cannot disagree; the 14
      showcase panels derive theirs from the panel registry via a new `panelSeo()`,
      so adding a panel needs no second edit; the remaining 9 pages are
      hand-written. robots.txt is hand-authored in `public/`. sitemap.xml is
      GENERATED by a Vite plugin (`src/app/seo/plugin.ts`) rather than committed —
      a checked-in sitemap is correct until the first route someone forgets, and
      one listing 404s is worse than none. The route parser moved out of the e2e
      helper into `src/app/seo/route-manifest.ts` so the sitemap and the route
      sweep cannot disagree about what the site contains.

      The gate is `e2e/seo.spec.ts`, wired into `ci:site` as `test:seo`. It does
      not check that a title exists — the root fallback would satisfy that
      silently, which is exactly how this shipped in the first place. It checks
      that each title is the route's OWN: unique across the site, not the
      fallback, under 70 characters, with a description, exactly one canonical
      pointing at the right URL, and the four OG tags. Verified green on a fresh
      build: 74 passed, and the route sweep still 72 passed.

      One latent bug surfaced and was fixed: `__root.tsx` used React's `charSet`
      where Solid's MetaHTMLAttributes names it `charset`. It had only ever
      typechecked by accident of how TS normalised that array literal.
