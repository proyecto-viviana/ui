...
const PlusIcon: Component = () => (
  <svg aria-hidden="true" viewBox="0 0 20 20" width="16" height="16" fill="none">
    <path d="M10 4v12M4 10h12" stroke="currentColor" stroke-width="1.75" />
  </svg>
);
...
function FixtureButton(props: FixtureButtonProps) {
  return (
    <Show
      when={props.shape === "square" || props.shape === "circle"}
      fallback={
        <Button
    ...
    >
      <Button
        ...
        loading={props.loading}
        onClick={props.onClick}
        shape={props.shape as "square" | "circle"}
        size={props.size}
        variant={props.variant}
      />
    </Show>
...
function FixtureGroup(props: { label: string; children: JSX.Element }) {
  return (
    <section class="kumo-fixture-group">
      <h2>{props.label}</h2>
      <div class="kumo-fixture-row">{props.children}</div>
    </section>
  );
}
...
export default function KumoButtonFixture() {
  const [state, setState] = createSignal<KumoButtonFixtureState>(KUMO_BUTTON_FIXTURE_DEFAULTS);
  ...
  return (
    ...
    >
      ...
      <FixtureGroup label="Form participation">
        <form
          id="kumo-native-form-solid"
          class="kumo-fixture-form"
      ...
      <FixtureGroup label="Pinned color modes">
        {KUMO_COLOR_MODES.map((mode) => (
          <div
            class="kumo-fixture-mode-swatch"
