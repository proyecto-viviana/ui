...
const entryOverrides: Record<string, ComparisonEntry> = {
  ...
  calendar: styledLiveOfficialEntry({
    ...
    styledNote:
      "React uses @react-spectrum/s2 Calendar directly; Solid uses @proyecto-viviana/solid-spectrum with the public Calendar subpath, CalendarContext export, S2 firstDayOfWeek string normalization, invalid selected-cell error description linkage, multi-month grid composition, pageBehavior/selectionAlignment state support, keyboard matrix parity, focus/reset state parity, Unicode display calendars, and custom createCalendar routing. Strict pair diff remains tracked.",
  }),
