...
| Solid Idiomatic Implementation           | passing | `packages/solid-spectrum/src/checkbox/index.tsx`, package tests.                                                                                          | Context and local props merge through Solid getters, refs merge without dropping caller refs, and the existing CheckboxGroup style context remains isolated for the later CheckboxGroup pass.                                                  |
...
- Hover/focus visual states are covered through current geometry/data-state
  ...
  grid.
- `CheckboxGroupContext` and CheckboxGroup default-state parity remain tracked
  for the separate CheckboxGroup pass.
