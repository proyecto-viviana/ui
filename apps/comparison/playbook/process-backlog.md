...
| Finish the S2 macro migration after the Vite Plus package-build checkpoint: convert `s2-style` imports to `with {type: 'macro'}`, run `macros.vite()` in the comparison app, then delete the runtime registry/generator once package and app CSS emission are proven exhaustive. | solid-spectrum build system  | complete S2 CSS parity     |
