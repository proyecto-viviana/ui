...
The official React Spectrum S2 component page is validation input, not just
background reading. If the official page exposes an example, option, state, or
accessibility note that our comparison route cannot represent, record that as a
gap. It is usually a signal that the port is incomplete.
...
For the target S2 docs page, record:
...
Do not mark Task 13 accepted while any in-scope official docs/viewer item is
