...
describe("Form hydrates over SSR markup", () => {
  ...
  it("Form+Button", async () => {
    const container = await hydrateOverSsr(readSsr("form-button-ssr.html"), () => (
      <FormButtonFixture />
    ));
  ...
  it("Form+TextField (profile shape)", async () => {
    const container = await hydrateOverSsr(readSsr("form-textfield-ssr.html"), () => (
      <FormTextFieldFixture />
    ));
  ...
  it("Form+TextArea (foros shape)", async () => {
    const container = await hydrateOverSsr(readSsr("form-textarea-ssr.html"), () => (
      <FormTextAreaFixture />
    ));
  ...
  it("Form+native button", async () => {
    await hydrateOverSsr(readSsr("form-native-button-ssr.html"), () => <FormNativeButtonFixture />);
  });

  it("Form+two Buttons", async () => {
    await hydrateOverSsr(readSsr("form-two-buttons-ssr.html"), () => <FormTwoButtonsFixture />);
  });

  it("Form+Button in fragment", async () => {
    await hydrateOverSsr(readSsr("form-button-fragment-ssr.html"), () => (
      <FormButtonInFragmentFixture />
    ));
  });

  it("Form+Picker (country picker shape)", async () => {
    const container = await hydrateOverSsr(readSsr("form-picker-ssr.html"), () => (
      <FormPickerFixture />
    ));
