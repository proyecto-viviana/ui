...
describe("ComboBox (solid-spectrum)", () => {
  ...
  // react-aria 3.52.0 `isVirtualClick`: `detail === 0` (keyboards, AT,
  // `element.click()`) is virtual and focus-visible. A mouse click is
  // `detail: 1` after `pointerdown` and is not. Both atoms read that answer
  // off the option render props.
  it("paints a synthetic detail-0 open as focus-visible", async () => {
    ...
    // Declaration atoms only. A leading `-` is a dev `-macro-dynamic-` marker
    // or a custom property such as `--iconPrimary`, whose forced-colors value
    ...
    expect(selected()).toHaveAttribute("data-focus-visible");
  ...
  it("does not paint a real mouse click as focus-visible", async () => {
    const user = setupUser();
    render(() => <FruitComboBox defaultSelectedKey="2" />);
    ...
    expect(focusedCheckmark.filter((t) => !restingCheckmark.includes(t))).toHaveLength(0);
    expect(focusedRow.filter((t) => !restingRow.includes(t))).toHaveLength(1);
  });
