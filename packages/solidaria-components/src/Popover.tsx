  ...
  return (
    <Show when={!isHidden()} fallback={hiddenChildren()}>
      <Show when={isHydrated() && (isOpen() || isExiting())}>
        {/* Solid 2's Portal splices its children into the mount between two
            text markers, so there is no wrapper element to style — the same
            shape as RAC's `createPortal`. */}
