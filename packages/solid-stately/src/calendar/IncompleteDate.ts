...
// Ported to SolidJS for Proyecto Viviana; based on packages/react-stately/src/datepicker/IncompleteDate.ts

/**
 * IncompleteDate for Solid-Stately
 *
 * Represents a date that is incomplete or otherwise invalid as a result of user
 * editing (for example February 31st, if the day is edited before the month).
 * This lets a date field hold an invalid intermediate value while typing and
 * only constrain it on blur, matching upstream behavior.
 *
 * Ported from packages/react-stately/src/datepicker/IncompleteDate.ts.
 */
...
import type { DateSegmentType } from "./createDateFieldState";
...
export class IncompleteDate {
  ...
  isComplete(segments: DateSegmentType[]): boolean {
    return segments.every((segment) => this[segment as keyof IncompleteDate] != null);
  }
  ...
  cycle(
    ...
    if (res[field as keyof IncompleteDate] == null && field !== "dayPeriod" && field !== "era") {
