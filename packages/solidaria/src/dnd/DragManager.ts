...
// Ported to SolidJS for Proyecto Viviana; based on packages/react-aria/src/dnd/DragManager.ts

/**
 * Global keyboard/virtual drag session manager.
 *
 * Faithful port of @react-aria/dnd `DragManager` (react-aria 3.50). A single
 * module-level singleton owns the active drag session: it registers drop targets
 * and drop items across every collection on the page, drives Tab/Shift+Tab
 * cycling between them, Enter-to-drop / Escape-to-cancel, and the aria-live
 * announcements — providing the accessible keyboard alternative to native
 * pointer drag and drop.
 *
 * The only framework-specific adaptation from upstream is the reactive
 * subscription: React's `useDragSession` (useState + a subscription Set) becomes
 * a module-level Solid signal exposed through `createDragSession()`. Element refs
...
import { getActiveElement, getEventTarget, nodeContains } from "../utils/dom";
import { getDragModality, getTypes } from "./utils";
import { isVirtualClick, isVirtualPointerEvent } from "../utils/events";
import type { LocalizedStringFormatter } from "@internationalized/string";

/** A Solid element accessor, replacing upstream's `RefObject<T>.current`. */
type ElementAccessor = () => HTMLElement | null;

let dropTargets = new Map<Element, DropTarget>();
let dropItems = new Map<Element, DroppableItem>();
let dragSession: DragSession | null = null;

// Reactive mirror of `dragSession` for `createDragSession()`. Solid's signal is
// the notification mechanism, replacing upstream's `subscriptions` Set.
const [trackedDragSession, setTrackedDragSession] = createSignal<DragSession | null>(null);
...
interface DropTarget {
  ...
  activateButtonRef?: ElementAccessor;
}
...
export function createDragSession(): Accessor<DragSession | null> {
  return trackedDragSession;
}
...
class DragSession {
  ...
  getCurrentActivateButton(): HTMLElement | null {
    return (
      this.currentDropItem?.activateButtonRef?.() ??
      this.currentDropTarget?.activateButtonRef?.() ??
      null
    );
  }
  ...
  onFocus(e: FocusEvent): void {
    ...
    let eventTarget = getEventTarget<HTMLElement>(e);
    ...
    if (isVirtualClick(e) || this.isVirtualClick) {
      ...
      let item = [...dropElements].find(
        (item) =>
          ...
          nodeContains(item.activateButtonRef?.() ?? null, eventTarget),
      );
      let dropTarget = this.validDropTargets.find((target) =>
        nodeContains(target.element, eventTarget),
      );
      let activateButton = item?.activateButtonRef?.() ?? dropTarget?.activateButtonRef?.() ?? null;
    ...
    this.restoreAriaHidden = ariaHideOutside(
      [
        ...
        ...validDropItems.flatMap((item) => {
          let button = item.activateButtonRef?.();
          return button ? [item.element, button] : [item.element];
        }),
        ...visibleDropTargets.flatMap((target) => {
          let button = target.activateButtonRef?.();
          return button ? [target.element, button] : [target.element];
        }),
      ],
      { shouldUseInert: true },
    );
...
function findValidDropTargets(options: DragTarget): DropTarget[] {
