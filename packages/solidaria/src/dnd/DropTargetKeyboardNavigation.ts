 ...
 * nested items.
 *
 * Upstream reads `node.nextKey` / `node.prevKey` (the flattened collection
 * linked-list pointers). In our collection model those are exactly what
 * `collection.getKeyAfter(key)` / `collection.getKeyBefore(key)` return, so they
 * are substituted 1:1 with no behavioral difference. Upstream's `lastChildKey`
 * fast-path is absent from our `CollectionNode`, so we always take upstream's
 * own `childNodes.findLast` fallback branch.
 */
...
/**
 * Keyboard delegate shape consumed by drop-target navigation. Mirrors the
 * subset of `@react-types/shared` `KeyboardDelegate` that navigation uses,
 * including the `{includeDisabled}` option so drop positions around disabled
 * items remain reachable.
 */
export interface DropNavigationKeyboardDelegate {
...
export function navigate(
  keyboardDelegate: DropNavigationKeyboardDelegate,
    ...
    switch (target.dropPosition) {
      ...
      case "after": {
        ...
        if (targetNode && nextItemInSameLevel == null && targetNode.parentKey != null) {
          ...
          let parentNextKey = parentNode != null ? collection.getKeyAfter(parentNode.key) : null;
          const nextNode = parentNextKey != null ? collection.getItem(parentNextKey) : null;
  ...
  if (target.type === "item") {
    ...
    switch (target.dropPosition) {
      case "before": {
        ...
        let targetNode = collection.getItem(target.key);
        let targetPrevKey = targetNode != null ? collection.getKeyBefore(targetNode.key) : null;
        if (targetNode && targetPrevKey != null) {
          let lastChild = getLastChild(collection, targetPrevKey);
          if (lastChild) {
            return lastChild;
          }
        }
