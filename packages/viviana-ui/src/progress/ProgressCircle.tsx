...
const trackStyles = style<ProgressCircleStyleState>({
  stroke: {
    /* Viviana UI v2 (Glasselated): the register draws every unfilled progress track on the
     * inset surface — the linear bar's track and the unlit dots of the pixel focus ring are
     * both `var(--surface-inset)`. `pasteboard` is that variable's name in the theme's colour
     * maps; the `stroke` map carries it under the same name (style/spectrum-theme.ts) so the
     * circular track lands on the same recess as the bar and Meter rather than the gray ramp. */
    default: "pasteboard",
