...
const dividerStyles = style<{
...
}>(
  {
    ...staticColor(),
    alignSelf: "stretch",
    /* The register paints its dividers as a filled span rather than drawing them as a
       border, which is what makes an edge token legal in `backgroundColor` at all —
       see the rule-weight entries in the backgroundColor map (style/spectrum-theme.ts),
       added for exactly this component. The gray ramp it replaces reads markedly too
       light in dark for a hairline.

       One colour serves every size. `size` still separates its values by thickness
       (1/2/4px in the height/width blocks below), and the token map deliberately
       publishes no third, heavier rule weight for L to be promoted to. */
    backgroundColor: {
    ...
    borderStyle: "none",
    /* No borderRadius: a rule is square in this register. The vertical divider is a
     * painted 1px span and the horizontal one a 1px edge — neither can or does round.
     * `full` was rendering the 2px default rule as a stadium. The sibling primitive in
     * separator/index.tsx declares no radius either. */
    margin: 0,
    flexGrow: 0,
    flexShrink: 0,
    height: {
