...
const selectBoxStyles = style<ListBoxOptionRenderProps & { orientation?: SelectBoxOrientation }>({
   ...
   * register's inset rim (style/spectrum-theme.ts), all three states resolve to the SAME
   * rim and the map no longer signals anything. The edge is where the register puts this
   * signal anyway: the handoff's own surface hover is a border treatment, not a lift.
   *
   * The resting stop also stops being `transparent` — a glass surface in this register
   * carries a visible 1px `--border-subtle` edge at rest, so hover/selection now read as a
   * change of edge COLOR rather than an edge appearing from nothing.
   *
   * `accent-900`/`accent-700` rather than a plain `accent`: the theme's borderColor map is
   * `...baseColors` plus negative/disabled and the three edge tokens, so it carries ramp
   * stops but no semantic `accent` key — that exists only on color/backgroundColor/fill.
   * Same constraint the tag-group edge hit. */
  borderColor: {
    default: "border-subtle",
    isHovered: "border-default",
