...
// Ported to SolidJS for Proyecto Viviana; based on packages/@react-spectrum/s2/src/Picker.tsx
...
const pickerOption = style<PickerOptionStyleProps>({
  ...
  // Glasselated: a register row is NEVER painted. The handoff's row primitive is
  // `.tgl-nav`, shared by the nav and the list rows, and all five of its rules are
  // ink or mark-opacity only (design-handoff-v2.css:434, :441, :445, :449, :453) —
  // there is no background-color in any of rest / hover / selected. The row
  // elements carry none inline either (TerminalGlassLab.tsx:593-602 nav rows,
  ...
  color: {
    default: baseColor("neutral"),
    // Full-strength `accent`, NOT the handoff's literal hover value
    // `color-mix(in srgb, var(--accent-primary) 55%, var(--text-tertiary))`
    // (design-handoff-v2.css:454). Two deliberate deviations. The mix is calibrated
    // for a nav row resting at `--text-tertiary` (NAV_ITEMS, TerminalGlassLab.tsx:67-69);
    // an option here rests at full-strength neutral, so the mix would DIM the label
    // on highlight. And full accent is the handoff's SELECTED-row ink
    // (NAV_ITEMS[0], :66), so borrowing it for highlight collapses a distinction the
    // nav draws — acceptable here only because a Picker option carries persistent
    // selection in the checkmark (`pickerCheckmark` below), not in its label color.
    isFocused: "accent",
    isDisabled: {
      default: "disabled",
      forcedColors: "GrayText",
