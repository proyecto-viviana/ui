...
const segmentedControl = style({
  ...
  /* No track. The register's segmented control is bare mono labels in a flex row —
     ...
     raw light-dark() literals, rgb(233,233,233) / rgb(44,44,44), which answer to no
     token in either scheme and so could not follow a theme flip. The radius goes with
     the fill: with nothing painted there is no corner to round. */
  width: "fit",
});

const segmentedControlItem = style<ToggleButtonRenderProps & { isJustified?: boolean }>({
  ...focusRing(),
  ...control({ shape: "default", icon: true, rim: false }),
...
/* No type parameter, and not called as a function below: every condition in this object is
   static (`forcedColors`), so the macro bakes it to a plain class string rather than to a
   selector function. Invoking it throws "selectionIndicator is not a function" at render.
   Re-adding a runtime-prop condition here means restoring the call form at the usage site. */
const selectionIndicator = style({
  ...
  /* The indicator paints nothing in either scheme: the register signals selection with
     ink, not with a bordered plate (TerminalGlassLab.tsx:443-459). The element is kept
     rather than deleted because it carries the translate/width transition above and,
     in forced-colors mode, is still the only surface available to mark the selection —
     that branch is preserved on both maps. Radius drops from `card` (12px, a container
     value) to `control` (5px), which is what `control()` gives the item itself, so the
     forced-colors mark traces the segment instead of overshooting it.
     `isDisabled` is dropped from both maps deliberately: over a transparent resting
     state a disabled-only border/fill would appear out of nowhere; the disabled label
     colour on the item above is the surviving signal. */
  borderStyle: "solid",
  borderWidth: 1,
  borderColor: {
    default: "transparent",
