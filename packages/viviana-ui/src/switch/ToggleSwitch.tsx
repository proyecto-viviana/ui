...
// Ported to SolidJS for Proyecto Viviana; based on packages/@react-spectrum/s2/src/Switch.tsx
...
// The track — structurally upstream's `track`, with one deliberate Glasselated
// divergence in the `backgroundColor.isSelected` VALUES (see the note there).
// The prior port carried a separate, self-inflicted divergence in that same
// block: a custom `disabledSelectedTrackBackground` light-dark value AND an
// `isDisabled`-first condition order (which, under last-match-wins, let disabled
// beat emphasized). Upstream's exact
// `{default, isEmphasized, forcedColors, isDisabled}` key order and `gray-400`
// disabled-selected value are kept — the order is load-bearing, so `isDisabled`
...
const track = style<SwitchStyleState>({
  ...
  backgroundColor: {
    ...
    isSelected: {
      ...
      // non-emphasized selected switch filled with the text ramp and rendered no
      // accent at all. `isEmphasized` is preserved as a real, visible step rather
      // than deleted: it goes one stop deeper in the same accent channel
      // (accent-1000), matching checkbox/index.tsx:407-408, so the public prop
      // keeps changing what it renders. Both stops are held flat rather than
      // moved to the lightDark("accent-900","accent-700") idiom used by
      // Button/Tag, because the handle riding this track is `gray-25` (below),
      // which flips to near-black in dark (style/glasselated-ramps.ts) — near-black
      // on dark accent-700 would not clear AA.
